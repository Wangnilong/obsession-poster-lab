...
import SiteHomeStudio from "./site-home-studio";
...
export default function ArchiveAdminPage() {
  ...
  useEffect(() => {
    const openClip = () => {
      ...
      setHomeMode(false); setWechatMode(true);
    };
    openClip();
  ...
  const navigate = (action: () => void) => {
    ...
    if (window.dispatchEvent(event)) { setHomeMode(false); setWechatMode(false); setOpenRecordId(""); action(); }
  };
  ...
  return <main className="archive-admin-workspace cms-workspace"><header className="archive-workspace-topbar"><a href="/archive/" className="archive-workspace-brand"><span>COSMOS</span><strong>宇宙放映内容台</strong></a><div className="archive-workspace-crumb">ISSUE {activeFilm.issue} · {activeFilm.zhTitle}</div><div className="archive-workspace-account"><strong>{editor.username}</strong><button type="button" onClick={() => navigate(() => { void signOutArchiveUser().then(() => setEditor(null)); })}>退出</button></div></header><div className="cms-shell"><aside className="archive-workspace-nav"><div className="archive-nav-heading"><strong>内容管理</strong></div>{editor.role === "admin" && <section><div className="archive-section-nav"><button type="button" className={homeMode ? "is-active" : ""} onClick={() => navigate(() => setHomeMode(true))}><strong>首页图片与文字</strong></button></div></section>}{editor.role === "admin" && <section><div className="archive-section-nav"><button type="button" className={eventsMode && !reviewMode && !wechatMode && !homeMode ? "is-active" : ""} onClick={() => navigate(() => { setEventsMode(true); setReviewMode(false); })}><strong>活动与日历 ＋</strong></button></div></section>}{editor.role === "admin" && <section><div className="archive-section-nav"><button type="button" className={reviewMode && !wechatMode && !homeMode ? "is-active" : ""} onClick={() => navigate(() => setReviewMode(true))}><strong>照片审核</strong></button></div></section>}{editor.role === "admin" && <section><div className="archive-section-nav"><button type="button" className={wechatMode && !homeMode ? "is-active" : ""} onClick={() => navigate(() => setWechatMode(true))}><strong>公众号转网页</strong></button></div></section>}{catalogError && <p role="alert">{catalogError}</p>}<section><h2>选择场次</h2><div className="archive-film-nav">{films.map(item => <button type="button" key={item.slug} className={film === item.slug && !community && !eventsMode && !reviewMode && !wechatMode && !homeMode ? "is-active" : ""} onClick={() => navigate(() => { setFilm(item.slug); setCommunity(false); setEventsMode(false); setReviewMode(false); })}><span>{item.issue}</span><strong>{item.zhTitle}</strong></button>)}</div></section><section><h2>活动内容</h2>{editor.role === "admin" && <div className="archive-section-nav"><button type="button" className={layoutMode && !community && !eventsMode && !reviewMode && !wechatMode && !homeMode ? "is-active" : ""} onClick={() => navigate(() => { setLayoutMode(true); setCommunity(false); setEventsMode(false); setReviewMode(false); })}><strong>页面布局</strong></button></div>}<div className="archive-section-nav">{(["photos", "articles", "merch", "tools"] as ArchiveSection[]).map((key, index) => <button type="button" key={key} disabled={editor.role === "photo-uploader" && key !== "photos"} className={!community && !layoutMode && !eventsMode && !reviewMode && !wechatMode && section === key ? "is-active" : ""} onClick={() => navigate(() => { setSection(key); setCommunity(false); setLayoutMode(false); setEventsMode(false); setReviewMode(false); })}><span>0{index + 1}</span><strong>{key === "articles" ? "图文内容" : archiveSectionLabels[key].zh}</strong></button>)}</div></section>{editor.role === "admin" && <section><h2>玩家作品</h2><div className="archive-section-nav"><button type="button" className={community && !eventsMode && !reviewMode && !wechatMode && !homeMode ? "is-active" : ""} onClick={() => navigate(() => { setCommunity(true); setEventsMode(false); setReviewMode(false); })}><strong>身份卡与暗杀名单</strong></button></div></section>}</aside>{homeMode && editor.role === "admin" ? <SiteHomeStudio /> : wechatMode && editor.role === "admin" ? <ArchiveWechatStudio films={films} initialFilm={activeFilm.slug} username={editor.username} onOpen={(slug, id) => navigate(() => { setOpenRecordId(id); setFilm(slug); setSection("articles"); setCommunity(false); setEventsMode(false); setReviewMode(false); setLayoutMode(false); })} /> : reviewMode && editor.role === "admin" ? <ArchivePhotoReview films={films} /> : eventsMode && editor.role === "admin" ? <ArchiveEventsStudio onSaved={value => { setFilms(value); setCatalogError(""); }} onOpen={(slug, section) => navigate(() => { setFilm(slug); setCommunity(false); setEventsMode(false); setReviewMode(false); setLayoutMode(!section); if (section) setSection(section); })} /> : community ? <CardCreationsAdmin role={editor.role} /> : layoutMode ? <ArchiveLayoutStudio key={film} film={film} activeFilm={activeFilm} /> : <ArchiveContentStudio initialRecordId={openRecordId} key={film + ":" + section + ":" + openRecordId} activeFilm={activeFilm} film={film} section={section} username={editor.username} role={editor.role} />}</div></main>;
}
