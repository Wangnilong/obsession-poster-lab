...
export default function ArchiveAdminPage() {
  ...
  return <main className="archive-admin-workspace cms-workspace"><header className="archive-workspace-topbar"><a href="/archive/" className="archive-workspace-brand"><span>COSMOS</span><strong>宇宙放映内容台</strong></a><div className="archive-workspace-crumb">ISSUE {activeFilm.issue} · {activeFilm.zhTitle}</div><div className="archive-workspace-account"><strong>{editor.username}</strong><button type="button" onClick={() => navigate(() => { void signOutArchiveUser().then(() => setEditor(null)); })}>退出</button></div></header><div className="cms-shell"><aside className="archive-workspace-nav"><div className="archive-nav-heading"><strong>内容管理</strong></div>{editor.role === "admin" && <section><div className="archive-section-nav"><button type="button" className={eventsMode ? "is-active" : ""} onClick={() => navigate(() => setEventsMode(true))}><strong>活动与日历 ＋</strong></button></div></section>}{catalogError && <p role="alert">{catalogError}</p>}<section><h2>选择场次</h2><div className="archive-film-nav">{films.map(item => <button type="button" key={item.slug} className={film === item.slug && !community && !eventsMode ? "is-active" : ""} onClick={() => navigate(() => { setFilm(item.slug); setCommunity(false); setEventsMode(false); })}><span>{item.issue}</span><strong>{item.zhTitle}</strong></button>)}</div></section><section><h2>活动内容</h2>{editor.role === "admin" && <div className="archive-section-nav"><button type="button" className={layoutMode && !community && !eventsMode ? "is-active" : ""} onClick={() => navigate(() => { setLayoutMode(true); setCommunity(false); setEventsMode(false); })}><strong>页面布局</strong></button></div>}<div className="archive-section-nav">{(["photos", "articles", "merch", "tools"] as ArchiveSection[]).map((key, index) => <button type="button" key={key} disabled={editor.role === "photo-uploader" && key !== "photos"} className={!community && !layoutMode && !eventsMode && section === key ? "is-active" : ""} onClick={() => navigate(() => { setSection(key); setCommunity(false); setLayoutMode(false); setEventsMode(false); })}><span>0{index + 1}</span><strong>{key === "articles" ? "图文内容" : archiveSectionLabels[key].zh}</strong></button>)}</div></section>{editor.role === "admin" && <section><h2>玩家作品</h2><div className="archive-section-nav"><button type="button" className={community && !eventsMode ? "is-active" : ""} onClick={() => navigate(() => { setCommunity(true); setEventsMode(false); })}><strong>身份卡与暗杀名单</strong></button></div></section>}</aside>{eventsMode && editor.role === "admin" ? <ArchiveEventsStudio onSaved={value => { setFilms(value); setCatalogError(""); }} onOpen={(slug, section) => navigate(() => { setFilm(slug); setCommunity(false); setEventsMode(false); setLayoutMode(!section); if (section) setSection(section); })} /> : community ? <CardCreationsAdmin role={editor.role} /> : layoutMode ? <ArchiveLayoutStudio key={film} film={film} activeFilm={activeFilm} /> : <ArchiveContentStudio key={film + ":" + section} activeFilm={activeFilm} film={film} section={section} username={editor.username} role={editor.role} />}</div></main>;
}
