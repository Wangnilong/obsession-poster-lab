...
function pasteHtml(html: string, text: string) {
  ...
  const safe = DOMPurify.sanitize(html, { ADD_ATTR: ["data-src"], RETURN_DOM_FRAGMENT: true });
...
export default function ArchiveWechatStudio({ films, initialFilm, username, onOpen }: { films: ArchiveFilm[]; initialFilm: string; username: string; onOpen: (film: string) => void }) {
  ...
  const change = () => { setConverted(null); setSavedId(""); setDirty(true); setMessage(""); setError(""); };
  ...
  const convert = async () => {
    setBusy(true); setError(""); setMessage("正在整理排版并保存图片…");
    try {
      const result = await convertWechatArticle({ film, mode, title, url, html });
      setConverted({ ...result, articleHtml: cleanEditorHtml(result.articleHtml), film }); setSavedId(""); setDirty(true);
      setMessage(`转换完成，已保存 ${result.images} 张图片。核对右侧网页后保存草稿。`);
